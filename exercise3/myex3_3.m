...
r_start = 75/1000; r_end = 105/1000; N_rp = 10; phi_r = 0*pi/180;
[phantom_positions, phantom_amplitudes] = ph_rect_gauss(phantom_count, ph_dim, origo);
%phantom_positions = [0/1000 0/1000 95/1000];
%phantom_amplitudes = [1];
addPsCross = 0;
if addPsCross
    % phantom_positions = [];
    % phantom_amplitudes= [];
    phantom_positions = [phantom_positions; [0 0 80]/1000; [0 0 90]/1000; [0 0 100]/1000; [-20 0 90]/1000; [20 0 90]/1000];
    ps_amp = 10^(25/20);
    phantom_amplitudes = [phantom_amplitudes; ones(5,1)*ps_amp];
end
...
N_samples = size(rf_d,1);

rf_tmp = nonzeros(rf_d(:));
figure;
hist(rf_tmp(:),100);
...
if show2dFourier
    ...
    Nffty = 256;            % Number ofkoao points in y dir for the FFT
...
tDemod = linspace(0,N_samples/fn,N_samples);
carrier = transpose(exp(-1i*2*pi*tDemod*f0));  %%

fn_y = linspace(0, fn, N_samples);
figure; plot(fn_y/1e6,abs(fft(carrier)));
title("minus carrier freq")

rf_d_analytic = hilbert(rf_d);
...
if show2dFourier
    ...
    plot2dFFT(rf_d_analytic, da, dr*1e6, Nfftx, Nffty);
    ylim([-6 6])         % Limit yaxis to -6MHz to 6 MHz
    xlabel('Spatial frequency [rad/s]');
    ylabel('Pulse frequency [MHz]');
    title('2D Fourier transform of analytic RF data');
end

carrier_mat = repmat(carrier, 1,N_lines);
iqData = rf_d_analytic .* carrier_mat;
...
if show2dFourier
    ...
    plot2dFFT(carrier_mat, da, dr*1e6, Nfftx, Nffty);
    ylim([-6 6])         % Limit yaxis to -6MHz to 6 MHz
    xlabel('Spatial frequency [rad/s]');
    ylabel('Pulse frequency [MHz]');
    title('2D Fourier transform of Carrier');
end
...
if show2dFourier
    ...
    plot2dFFT(iqData, da, dr*1e6, Nfftx, Nffty);
    ylim([-6 6])         % Limit yaxis to -6MHz to 6 MHz
    xlabel('Spatial frequency [rad/s]');
    ylabel('Pulse frequency [MHz]');
    title('2D Fourier transform of iqData');
end

figure;
for i=1:N_lines
    hold on;
    plot(fn_y/1e6, abs(fft(iqData(:,i))));
end
title("iqData freq spectrum");

figure;
for i=1:N_lines
    hold on;
    plot(fn_y/1e6, abs(fft(rf_d(:,i))));
end
title("RF Data freq spectrum");

figure;
for i=1:N_lines
    hold on;
    plot(fn_y/1e6, abs(fft(carrier_mat(:,i))));
end
title("Carrier freq spectrum");

theDet = iqData .* conj(iqData);
theDet = theDet/max(max(theDet))+1e-12;

theDetComp = 10*log10(theDet);
...
if show2dFourier
    ...
    plot2dFFT(theDet, da, dr*1e6, Nfftx, Nffty);
    ylim([-6 6])         % Limit yaxis to -6MHz to 6 MHz
    xlabel('Spatial frequency [rad/s]');
    ylabel('Pulse frequency [MHz]');
    title('2D Fourier transform of detection Data');
end
...
% Plot image in beam space
figure;
h_ph_img = imagesc(x_axis_angle*180/pi, disttxrx*1000, theDetComp);
caxis([-dyn 0]-gain);
xlabel(['Beam angle [deg]'])
ylabel('Range [mm]')
colormap(gray(256));
s_ratio = sprintf("oversamplingratio %2g, N_lines %2g",oversamplingratio, N_lines);
title(s_ratio,'Interpreter','none');
axis on
...
figure; 
pcolor(X_im*1000,Y_im*1000,theDetComp); 
