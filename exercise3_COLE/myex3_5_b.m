% this script is to simulate the scan line movement 
% this movement is from left to right
% similar like the real scan sequence
...
lateral_lut = [0 3]/1000; 

axial_fov = [20 100]/1000;
lateral_fov = [-30 30]/1000; 
...
if ~exist('LUT_1.mat','file')
    ...
    focus = [0 0 90]/1000;      % Fixed emitter focal point [m] (irrelevant for single element transducer)
    N_elements = 32;            % Number of physical elements in array
...
nscatterers = 2000;
nframes = 30;
% move the scatter block from the left to right
% dy = 50/1000;
dy = 0;
% dx = 0/1000;
dx = 30/1000;
...
side_y = 20/1000;
offset = 50;
x_scatt = rand(1,nscatterers) * side_x - 20/1000;
y_scatt = rand(1,nscatterers) * side_y + offset/1000;
amp = 1;

% Prepare the new file.
% thisscriptname = mfilename;
thisscriptname = sprintf("%s_%d",mfilename,offset);
vidObj = VideoWriter(thisscriptname.char);
open(vidObj);
...
%        plot3(surf_ax,d,y_scatt,w_amp,'*');
    ...
    % re-generate the scatter point
    % x_scatt = rand(1,nscatterers) * side_x - side_x;
