...
N_sub_y = 5;                % Element subdivision in y-direction
...
excitation=1*square(2*pi*f0*t_ex);
...
% [phantom_positions, phantom_amplitudes] = cyst_phantom_thin(phantom_count);
% [phantom_positions, phantom_amplitudes] = point_scatterer(focus)
[phantom_positions, phantom_amplitudes] = ph_rect_gauss(phantom_count, ph_dim, origo);
% [phantom_positions, phantom_amplitudes] = ph_rect_gauss_regular(phantom_count, ph_dim, origo);
% [phantom_positions, phantom_amplitudes] = rline_scatterers(r_start, r_end, N_rp, phi_r)
% [phantom_positions, phantom_amplitudes] = xline_scatterers(-5/1000, 5/1000, 50, 70/1000)
...
% Plot 2D Fourier transform after hilbert (pre-envelope)
if show2dFourier   
    plot2dFFT(rf_h, da, dr*1e6, Nfftx, Nffty);
    ylim([-6 6])         % Limit yaxis to -6 to 6 MHz
    xlabel('Spatial frequency [rad/s]');
    ylabel('Pulse frequency [MHz]');
    title('2D Fourier transform of pre-envelope data');
end
...
% Plot 2D Fourier transform after iq-demodulation
if show2dFourier   
    plot2dFFT(iqData, da, dr*1e6, Nfftx, Nffty);
    ylim([-6 6])         % Limit yaxis to -6 to 6 MHz
    xlabel('Spatial frequency [rad/s]');
    ylabel('Pulse frequency [MHz]');
    title('2D Fourier transform of IQ data');
end
...
% Plot 2D Fourier transform after iq-demodulation
if show2dFourier   
    plot2dFFT(env, da, dr*1e6, Nfftx, Nffty);
    ylim([-6 6])         % Limit yaxis to 0 to 6 MHz
    xlabel('Spatial frequency [rad/s]');
    ylabel('Pulse frequency [MHz]');
    title('2D Fourier transform of detected envelope data');
end
